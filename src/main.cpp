...
void copy_screenshot(std::unique_ptr<uint8_t[]> data, const CCSize& size, bool copy, uint x, uint y, uint a, uint b) {
    ...
    std::thread([=, data = std::move(data)]() {
        if (copy) {
			...
			image->initWithImageData(data, (int)size.width * (int)size.height * 4, eSrcFmtPng, (int)size.width, (int)size.height, 8);
			image->saveToFile(geode::Mod::get()->getConfigDir() / "test.png", true);
		}
    }).detach();
}
